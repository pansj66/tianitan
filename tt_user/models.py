from django.db import models
from db.base_model import BaseModel
from utils.get_hash import get_hash
# from utils import get_hash   BUG : 当模块名跟函数名相同,引用的时候一定要注意引用全了


# Create your models here.


class PassportManager(models.Manager):
    '''用户账户模型管理器类'''
    def add_passport(self, username, password, email):

        passport = self.create(username=username, password=get_hash(password), email=email)
        return passport

    def get_passport(self, username, password):
        # print(type(password))
        # print(self.get(username=username).password)
        try:

            passport = self.get(password=get_hash(password),username=username)
        except self.model.DoesNotExist:
            passport = None
        return passport

class Passport(BaseModel):
    '''用户账户类'''
    username = models.CharField(max_length=20, verbose_name='用户名')
    password = models.CharField(max_length=40, verbose_name='密码')
    email = models.EmailField(verbose_name='邮箱')
    is_activate = models.BooleanField(default=False, validators='激活标记')

    objects = PassportManager()


    class Meta:
        db_table = 's_user_account'



class AddressManager(models.Manager):
    '''地址模型管理器类'''
    def get_default_address(self, passport_id):
        '''查询指定用户的默认收货地址'''
        try:
            addr = self.get(passport_id=passport_id, is_default=True)
        except self.model.DoesNotExist:
            addr = None
        return addr

    def add_address(self, passport_id, recipient_name, recipient_addr, zip_code, recipient_phone):
        '''添加收货地址'''
        addr = self.get_default_address(passport_id=passport_id)

        if addr:
            # 存在默认地址
            print('没有添加到默认地址')
            is_default = False

        else:
            # 不存在默认地址
            is_default = True
            print('添加了一个默认地址')
        addr = self.create(passport_id=passport_id,
                           recipient_phone=recipient_phone,
                           recipient_addr=recipient_addr,
                           recipient_name=recipient_name,
                           zip_code=zip_code,
                           is_default=is_default)

        return addr


class Address(BaseModel):
    '''地址模型类'''
    recipient_name = models.CharField(max_length=20, verbose_name='收件人')
    recipient_addr = models.CharField(max_length=256, verbose_name='收件地址')
    zip_code = models.CharField(max_length=6, verbose_name='邮政编码')
    recipient_phone = models.CharField(max_length=11, verbose_name='联系电话')
    is_default = models.BooleanField(default= False, verbose_name='是否为默认地址')
    passport = models.ForeignKey('Passport', verbose_name='账户')

    objects = AddressManager()

    class Meta:
        db_table = 's_user_address'









































